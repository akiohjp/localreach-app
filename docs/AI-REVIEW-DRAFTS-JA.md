...
店名、ゲストが ON にしたフレーズ（**店に設定済みのフレーズ以外は捨てる**）、ゲストの 1 行、エンティティ（業種名詞・エリア・都市）。訪問の中身は一切でっち上げない指示にしており、**選んだフレーズはすべて原文どおり**に入っていないと却下される（`lib/review-ai-filter.ts`）。
...
| 時間 | ルート全体 7.5 秒、1 試行 4.5 秒。クライアントは 9 秒で諦める |
