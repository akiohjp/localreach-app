...
却下条件: 長さレール外／引用符・絵文字・ハッシュタグ・星数／AI 臭い語（hidden gem, nestled, elevate など。ただしフレーズに含まれていれば可）／店名 2 回以上／連絡先。星 4 も星 5 と同じく肯定的に書く（4 は「よかった」の評価であって不満ではない。Akio の方針、2026-09-06。これはプロンプトの指示だけで担保し、留保語のフィルタは置かない）。却下は 1 回だけ作り直し、それでも駄目なら 502 でテンプレートに戻る。
