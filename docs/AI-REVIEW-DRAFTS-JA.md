...
却下条件: 長さレール外／引用符・絵文字・ハッシュタグ・星数／AI 臭い語（hidden gem, nestled, elevate など。ただしフレーズに含まれていれば可）／店名 2 回以上／連絡先。星 4 も星 5 と同じく肯定的に書く（4 は「よかった」の評価であって不満ではない。Akio の方針、2026-09-06。これはプロンプトの指示だけで担保し、留保語のフィルタは置かない）。店別の禁止語は 2 段階: 産地語（Cinar の Persian など）は文全体で不可、carpet のような「店は言わないが客は検索する」語は店が設定した検索フレーズの中だけ可で、それ以外の場所に出たら却下（2026-09-06、Akio 指示）。却下は最大 2 回作り直す（合計 3 回、7.5 秒の予算内）。それでも駄目なとき、却下理由が「長さ」「書き出し被り」「類似」だけの候補があれば、テンプレートではなくその候補を返す（記録は outcome=ai、reason=soft:…）。内容の却下（禁止語・連絡先・星数・AI 臭い語・フレーズ欠落）は返さず、502 でテンプレートに戻る。2026-09-08: 新インスタンスの初回コールで Google 側が遅い 1 分に当たり、38 語の下書きが長さで落ちて作り直しの時間が残らず、使える下書きがあるのにテンプレートに落ちたため。

## 被らせない仕組み（2026-09-07）

同じ店に何十本も溜まったとき「同じ人が書いた」と見えないように、3 段構え。

1. **書き出し 48 型・締め 24 型**を店ごとに順番に回す（総本数 + 試行回数で決めるので、最初の 48 人は全員違う型）。「If you are looking for」「I finally found」「店名」「I」で始める常套句と、「will definitely be back」「highly recommend」で終わる常套句は禁止。
2. その店の**直近 20 本の書き出し**をプロンプトに渡し、「これらと同じ始め方をしない」と指示する。
3. 出来た文の**冒頭 5 語が直近と一致**、または **4 語連なりの 3 割超が直近のどれかと重なる**なら却下して別の型で作り直す（`opening_repeat` / `too_similar`）。

ログ全体の検査: `npm run ai:diversity`（店ごとに書き出しの重複と最も似ている 2 本を出す。重複があれば exit 1）。
