...
const { generateReview, reviewKeywordForm } = await import("../lib/assembler.ts");
...
for (const s of rows) {
  ...
  for (let i=0;i<200;i++){
    ...
    checked++;
    // The draft carries a sentence-cased attribute in lower case ("the
    // long-lasting scent"); that IS the promised form (see reviewKeywordForm).
    for (const kw of uniq) { const shown = reviewKeywordForm(kw, s.keyword_types?.[kw]); if(!t.includes(shown)){ lost++; if(lost<=5) console.log(`LOST "${kw}" | ${name} | taps=${JSON.stringify(uniq)}\n  ${t}\n`); } }
  }
}
