...
const COUNT_PLACE_HEADS: ReadonlySet<string> = new Set([
  ...
  // Trade and venue nouns that are just as countable (audit of all 30 live
  // stores, 2026-09-15): "steakhouse in DIFC", "izakaya in Dubai", "Japanese
  // food supplier in Dubai", "koi pond builder in Dubai" all need the article.
  // Mass nouns stay OUT on purpose: sushi, ramen, coffee, beef, tea, matcha,
  // udon, pizza, gear, marketing, maintenance take no article and never did.
  "steakhouse", "izakaya", "ramen-ya", "teahouse", "tearoom", "brasserie",
  "trattoria", "patisserie", "creperie", "taverna", "grillhouse", "chophouse",
 ...
 */
/**
 * CATEGORY frames are noun-only: "They know their {kw}", "a good variety of
 * {kw}". A phrase that starts with an article, a preposition or a gerund
 * clause comes out broken in them — "They really know their a family dinner.",
 * "I came for with a group and was not disappointed.", "They keep a strong
 * selection of opening a restaurant." (naturalness gate over Rowley's, Maru
 * Udon, Summit Trading and Koi Water Barn, 2026-09-15: 5 to 7 rejects per 16).
 *
 * Owners type occasions as categories and always will, so the engine absorbs
 * it rather than relying on clean data: an unfit phrase leaves the category
 * pool and goes out through the appositive tails, which take ANY shape. Same
 * treatment attribute phrases have had since 2026-07-29.
 */
const CATEGORY_FRAME_UNFIT =
  /^(a|an|the|my|our|your|this|that)\s|^(with|for|before|after|during|without|from|by|inside|around)\s|^\w+ing\s+(a|an|the|my|our|your|this|that|to|into|within|from|around|in|at|on|for)\b/i;

function fitsCategoryFrame(phrase: string, locale: ReviewLocale): boolean {
  if (locale !== "en") return true;
  return !CATEGORY_FRAME_UNFIT.test(phrase.trim());
}

/**
 * The three quantity frames ("plenty of X to choose from", "a good variety of
 * X", "a strong selection of X") need a PLURAL or a mass noun. With a singular
 * occasion they read "They have plenty of weekend lunch to pick from."
 */
const CATEGORY_MASS_HEADS = new Set([
...
export function buildLocalizedReview(
  ...
  const typeOf = (k: string): KeywordType => {
    const t = classifyKeyword(k, keywordTypes, locale);
    // An occasion typed as a category ("a family dinner", "with a group",
    // "restocking the pond") fits neither the noun-only category frames nor an
    // object slot ("the restocking the pond"). Attribute is the shape that
    // takes anything: it leaves through the appositive tails.
    if (t === "category" && !fitsCategoryFrame(k, locale)) return "attribute";
    return t;
  };
  ...
  // An occasion typed as a category ("a family dinner", "with a group",
  // "opening a restaurant") cannot sit in a noun-only frame. It leaves the
  // category pool here and goes out through the appositive tails instead.
  ...
  // Quantity frames only where the head is plural or a mass noun.
  {
    ...
    if (singular.length) {
      ...
      weaveDedicated(singular, noQuantity.length ? noQuantity : catPool, 0x9e12);
    }
  }
