...
const EN_GENERIC: PoolSet = {
  // Pool sizes are a naturalness requirement, not padding: on a page with 100
  // reviews each distinctive line shows up ~100/N times, and readers notice a
  // repeated sentence long before a repeated structure (measured 2026-08-03:
  // at the old sizes the top filler hit 14x/100). Additions vary the SHAPE
  // (who is speaking to whom, what triggered the review, tense) — not just
  // the words.
  openersLong: [
    ...
    "{Really good|Genuinely great} experience at {store}, and I {don't usually bother leaving|almost never write} reviews.",
  ...
  coresLong: [
    ...
    "If I had to pick one thing, it'd be {list}.",
  ...
  dualBlocks: [
    ...
    "Hard to split {a} and {b}, both delivered.",
    "{a} was the plan, {b} was the bonus.",
    "Started with {a}, stayed for {b}.",
  ],
  // {a|b|c} groups are inline choices, resolved per review (see expandChoices
  ...
  bridgesLong: [
    ...
    "The person looking after us {remembered everything|kept track of everything|missed nothing} without writing it down.",
  ...
  fillers: [
    ...
    "I kept {waiting for|looking for|expecting} the catch and there {wasn't one|never was one|just wasn't}.",
  ...
  // Tails are the HOTTEST slot (one per leftover keyword, so ~2 uses per
  // review = ~200 draws per 100 reviews). Pool size matters most here, and the
  // CONSTANT part of each template is kept short — the keyword provides the
  // variety, so a repeated frame with a different keyword inside reads like a
  // person, while a long distinctive constant sentence repeated 10x reads like
...
const JA_GENERIC: PoolSet = {
  openersLong: [
    "先日{store}に行ってきました。また来たいと思える良いお店でした。",
    "気になっていた{store}にようやく行けました。行って良かったです。",
    "週末に{store}へ行ってきました。良い時間を過ごせて満足です。",
    ...
    "今週初めて{store}に行きましたが、期待以上でした。",
    "{store}の対応がとても良かったので、普段は書かないレビューを残します。",
    "家族で{store}に行きましたが、みんな満足して帰りました。",
    "{store}は人にも紹介したくなる、地元の良いお店です。",
    "同僚が何度も{store}の話をするので、自分の目で確かめてきました。",
    "たまたま通りかかって{store}に入りましたが、これが正解でした。",
    "気づけば今月2回目の{store}です。それくらい気に入っています。",
    "ちょっとしたお祝いで{store}を選びましたが、良い選択でした。",
    "前から気になっていた{store}、やっと行けたので記録しておきます。",
    "遠方から来た友人を{store}に連れて行きましたが、二人とも大満足でした。",
    "何ヶ月も前を素通りしていた{store}、もっと早く入ればよかったです。",
    "今回は家族の希望で{store}にしましたが、選んでくれて感謝です。",
    "疲れた日の{store}は格別でした。",
    "口コミを見て{store}を選びましたが、評判に嘘はありませんでした。",
  ],
  openersShort: [
    "{store}、とても良かったです。",
    "{store}、また行きます。",
  ...
  coresLong: [
    "特に{list}が良かったです。",
    "{list}が{一番の魅力|いちばんの決め手|一番の当たり}でした。",
    "{list}が目当てでしたが、期待どおりでした。",
    "{store}は{list}が本当にしっかりしていました。",
    "行って一番良かったのは{list}です。",
    "また来たい理由は{list}です。",
    "{list}がとても気に入りました。",
    "{list}は誰かに教えたくなる良さでした。",
    "何より{list}が印象に残りました。",
    "{list}は思っていた以上に良かったです。",
    "ひとつ挙げるなら、やはり{list}です。",
    "今回の主役は{list}でした。",
    "帰ってからも{list}の話をしていました。",
    "{list}は評判になるだけのことはあります。",
    "気になっていた{list}、実際に良かったです。",
    "私としては{list}で行く価値がありました。",
  ],
  coresCompact: [
    "{list}が良かったです。",
    "{list}、最高でした。",
  ...
  dualBlocks: [
    ...
    "{a}目当てで行って、{b}のファンになって帰ってきました。",
    "{a}と{b}、甲乙つけがたいです。",
    "{a}が本命で、{b}は嬉しいおまけでした。",
    "{a}から入って{b}で締める流れが良かったです。",
  ],
  bridgesLong: [
    ...
    "最初から最後まで{スムーズ|滞りなく快適|流れがきれい}でした。",
    "細かいところまで{気を配っている|行き届いている|丁寧にされている}のが伝わりました。",
    "{落ち着いた|穏やかな|ゆったりした}雰囲気で、急かされる感じもありませんでした。",
    "混んでいましたが、対応が{乱れることはありませんでした|雑になることはなかったです|落ち着いたままでした}。",
    "ちょうどいいタイミングで声をかけてくれるのが{心地よかった|ありがたかった|うれしかった}です。",
    "質問にも{はっきり|分かりやすく|丁寧に}答えてくれて、安心してお任せできました。",
    "できないことはできないと正直に言ってくれるのも{好印象|信頼できるポイント|良かった点}でした。",
    "初めてでも{居心地の悪さ|入りにくさ|よそ者感}をまったく感じませんでした。",
    "こちらのペースに合わせてくれるのが{ありがたかった|心地よかった|うれしかった}です。",
    "案内から会計まで、流れが{きれいに整っていました|よく考えられていました|スムーズでした}。",
    "{平日でも週末でも|いつ行っても|誰が行っても}同じ質なんだろうなと感じさせる安定感でした。",
  ],
  bridgesShort: [
    "接客も{親切で早かった|丁寧でスピーディー|感じ良くて早かった}です。",
    "{清潔で居心地良し|店内きれいで快適|清潔感があって快適}。",
    "値段も{良心的|納得感あり|きちんと適正}でした。",
    "スタッフが感じ良かったです。",
    "{最初から最後まで|入りから会計まで|終始}快適でした。",
    "{無駄がなくてスムーズ|流れが良くて快適|テンポが良くて楽}でした。",
    "対応が気持ちよかったです。",
    "手際が良くて驚きました。",
    "雰囲気も{落ち着いていました|穏やかで良かったです|ゆったりしていました}。",
    "気遣いが{自然|さりげなくて良かった|押しつけがましくなかった}です。",
  ],
  closersLong: [
    "{store}は{自信を持って|安心して|誰にでも}おすすめできます。",
    "また{store}に行きます。",
    "迷っているなら{store}で{間違いない|良い|外さない}と思います。",
    "{store}は{行きつけ|定番|通うお店}にします。",
    "まだ{store}に行ったことがないなら、一度どうぞ。",
    "{store}は本当におすすめです。",
    "{store}の常連になりそうです。",
    "次に{store}へ行くのが{楽しみ|今から楽しみ|待ち遠しい}です。",
    "帰ってから{もう2人|さっそく友人|家族}に{store}の話をしました。",
    "今度は{store}で{他のメニュー|別の時間帯|違うサービス}も試すつもりです。",
    "人を連れて行くなら{store}と決めました。",
    "こういう{お店|場所}が近くにあるのは、素直に{うれしい|心強い|ありがたい}です。",
    "{行こうか迷っていた時間|様子見していた期間|後回しにしていた時間}がもったいなかったです。",
    "次は待たずに{store}へ行きます。",
    "{大事な日|人を連れて行く日|ここぞという時}にも安心して使えるお店だと思います。",
    "{store}、また必ず。",
  ],
  closersShort: [
    "{store}、強くおすすめします。",
    "また{来ます|寄ります|お世話になります}。",
    "{store}、おすすめです。",
    "{store}をぜひ。",
    "全体的に良かったです。",
    "また{近いうちに|そのうち|折を見て}。",
  ...
  fillers: [
    ...
    "ちょっとした心遣いが嬉しかったです。",
    "期待していたとおりでした。",
    "これだけ安定していると、また来ようと思えます。",
    "友人のすすめで行きましたが、正解でした。",
    "働いている方が楽しそうなのも良かったです。",
    "ゆっくり過ごせたのも良かったです。",
    "内容を考えるとコスパも良いと思います。",
    "正直、これといった不満はありません。",
    "口コミどおりのお店って意外と少ないので、貴重だと思います。",
    "どこかに粗がないか探してしまいましたが、見つかりませんでした。",
    "この安定感は簡単に出せるものではないと思います。",
    "仕事に誇りを持っているのが伝わってくるお店です。",
    "細かいことが気になる性格ですが、指摘するところがありませんでした。",
    "ハードルを上げて行ったのに、それでも超えてきました。",
    "帰ってから家族に真っ先に話しました。",
    "当たり前のことを当たり前にやってくれる、それが一番です。",
    "行く前より気分よく帰れるお店は貴重です。",
    "近所にあったら通いすぎてしまいそうです。",
    "初めてでもリラックスして過ごせました。",
    "気づいたら滞在時間が予定より長くなっていました。",
  ],
  tails: [
    "{kw}も良かったです。",
    "{kw}はぜひ試してほしいです。",
  ...
  noKeywordMid: [
    "対応が良くて、それだけでもまた{store}に来たいと思えました。",
    "{store}は当たり前のことをきちんとやってくれる良いお店でした。",
    "対応してくれた方がみなさん親切で、気持ちよく過ごせました。",
    "入ってから帰るまで、ずっと快適でした。",
...
const AR_GENERIC: PoolSet = {
  openersLong: [
    "زرت {store} لأول مرة {وسأعود بالتأكيد|ولن تكون الأخيرة|وسأكرر الزيارة}.",
    "كنت أنوي تجربة {store} منذ فترة، وأخيراً فعلت، وأنا {سعيد بذلك|مسرور بذلك|لا أندم}.",
    "مررت على {store} في نهاية الأسبوع وكانت زيارة {جيدة حقاً|موفقة|ممتعة}.",
    "أول زيارة لي إلى {store} كانت عند حسن الظن كما {أخبرني الأصدقاء|قيل لي|سمعت}.",
    "قضيت وقتاً {ممتعاً|طيباً|جميلاً} في {store}، وأترك هذا التقييم ليجرّبه غيري.",
    "ذهبت إلى {store} من دون توقعات كبيرة وخرجت {سعيداً جداً|راضياً تماماً|معجباً}.",
    "{أوصاني أحدهم|نصحني صديق|رشّح لي زميل} بـ {store}، والآن أفهم سبب الإشادة به.",
    "مررت على {store} {اليوم|صباح اليوم|قبل قليل} وكان يستحق الزيارة.",
    "جرّبت {store} لأول مرة {هذا الأسبوع|قبل أيام|مؤخراً} ولم يخيّب ظني.",
    "تجربة {جيدة جداً|ممتازة|موفقة} في {store}، ولا أكتب التقييمات عادةً.",
    "زرت {store} مع {العائلة|الأهل|الأصدقاء} وخرجنا جميعاً سعداء.",
    "{store} من الأماكن المحلية التي {سأوصي بها الجميع|أنصح بها من دون تردد|أذكرها لكل من يسأل}.",
    "{زميلي في العمل|أحد الأصدقاء|جاري} ذكر {store} أكثر من مرة، فذهبت لأرى بنفسي.",
    "وصلت إلى {store} بالصدفة تقريباً، وكانت صدفة {موفقة|سعيدة|في محلها}.",
    "هذه زيارتي {الثانية|الثالثة} إلى {store} هذا الشهر، وهذا يقول كل شيء.",
    "اخترنا {store} لمناسبة {صغيرة|عائلية|بسيطة} وكان اختياراً موفقاً.",
    "كان عندي {وقت فراغ|بعض الوقت} في المنطقة فجرّبت {store}.",
    "بعد يوم {طويل|مزدحم|متعب}، كان {store} تماماً ما أحتاجه.",
    "أخذت {صديقاً زائراً|ضيفاً من خارج المدينة} إلى {store} وخرجنا كلانا معجبين.",
    "قرأت التقييمات قبل زيارة {store}، وتبيّن أنها {صادقة|في محلها|دقيقة}.",
  ],
  openersShort: [
    "زيارة {رائعة|ممتازة|موفقة} إلى {store}.",
    "{store}؟ سأعود بالتأكيد.",
    "{سعيد جداً|مسرور جداً|راضٍ تماماً} بـ {store}.",
    "زيارة {متينة|موفقة} إلى {store}، بلا ملاحظات.",
    "{store} لم يخيّب ظني.",
    "خرجت من {store} {بمزاج جيد|وأنا مرتاح|راضياً}.",
    "أول زيارة إلى {store} {وأنا معجب|وقد أعجبني|وسأعود}.",
    "كلمة {سريعة|قصيرة} عن {store}.",
    "{store} كما هو دائماً، {ممتاز|على مستواه|لم يتغير}.",
    "أخيراً جرّبت {store}، وكان قراراً {صائباً|موفقاً}.",
    "{store} عند حسن الظن.",
    "تجربة {مريحة|سلسة|مريحة جداً} مع {store}.",
  ],
  coresLong: [
    ...
    "لو اخترت شيئاً واحداً لقلت {list}.",
    "الزيارة كلها تلخصت في {list} بالنسبة لي.",
    "ما زلنا نتحدث عن {list}.",
    "{list} يستحق الضجة حوله.",
    "نجم الزيارة كان {list} بلا منازع.",
    "جئت أختبر {list} وخرجت مقتنعاً.",
  ],
  coresCompact: [
    "أحببت {list}.",
    "{list} كان ممتازاً.",
  ...
  bridgesLong: [
    "كان الطاقم {ودوداً وسريعاً|لطيفاً وسريعاً|مرحباً وسريعاً}، بلا انتظار.",
    "المكان {نظيف ومريح|مرتب ومريح|نظيف وهادئ}، من السهل {الاستقرار فيه|الجلوس فيه|قضاء الوقت فيه}.",
    "الأسعار {عادلة|معقولة|منصفة} مقابل ما تحصل عليه.",
    "كانت الخدمة {لطيفة|مهذبة|راقية} من دون إلحاح.",
    "لبّوا طلباً صغيراً من دون {أي عناء|تردد|تعقيد}.",
    "سار كل شيء {بسلاسة|على ما يرام|من دون عقبات} من البداية إلى النهاية.",
    "{يظهر|يبدو|من الواضح} أن الفريق يهتم بالتفاصيل.",
    "أجواء {هادئة ومريحة|مريحة وهادئة|بسيطة ومريحة}، بلا استعجال.",
    "رغم الازدحام، بقيت الخدمة {منظمة وهادئة|منضبطة|على مستواها}.",
    "أجابوا عن أسئلتي بوضوح ومن دون {مبالغة في العرض|ضغط|تهويل}.",
    "كانوا صريحين بشأن ما هو متاح وما ليس متاحاً، وهذا {يُحترم|أمر يُقدَّر|شيء يستحق التقدير}.",
    "التعامل كان {مهذباً وطبيعياً|بسيطاً ومهذباً|ودياً وطبيعياً}، من دون تكلّف.",
    "شعرت أن المستوى نفسه {يتكرر|يبقى ثابتاً|لا يتغير} في كل زيارة.",
    "اهتموا بنا من الدخول حتى الخروج من دون {مبالغة|تكلف|إفراط}.",
  ],
  bridgesShort: [
    "خدمة ودودة وسريعة.",
    "نظيف ومريح.",
  ...
  closersLong: [
    "أوصي بـ {store} {للجميع بكل سرور|لأي شخص بكل سرور|من دون تردد}.",
    "سأعود إلى {store} {بالتأكيد|بلا شك|من دون شك}.",
    "{store} خيار {متين|موثوق|جيد}، جرّبه.",
    "أضفت {store} إلى {قائمتي المعتادة|أماكني الثابتة|قائمتي المفضلة}.",
    "{إن|إذا} لم تجرّب {store} بعد، فافعل.",
    "لا أستطيع أن {أوصي بـ|أمدح} {store} بما يكفي.",
    "أصبحت {زبوناً دائماً|من الزبائن الدائمين} لـ {store}.",
    "أخطط {بالفعل|من الآن} لزيارتي القادمة إلى {store}.",
    "حدّثت {اثنين من أصدقائي|بعض أصدقائي|أكثر من صديق} عن {store} منذ زيارتي.",
    "في المرة القادمة لن أنتظر طويلاً قبل {العودة إلى|زيارة} {store}.",
    "سأصطحب {ضيوفي|أهلي|أصدقائي} إلى {store} من الآن فصاعداً.",
    "{store} {استحق|يستحق} هذا التقييم عن جدارة.",
    "خرجت من {store} {سعيداً أنني جئت|راضياً تماماً|وأنا مسرور}.",
    "صار {store} {خياري الجاهز|جوابي الجاهز} عندما يسألني أحد.",
  ],
  closersShort: [
    "أوصي بشدة بـ {store}.",
    "سأعود.",
  ...
  fillers: [
    ...
    "قليلة هي الأماكن التي تطابق تقييماتها، وهذا منها.",
    "كنت أنتظر أي هفوة ولم أجد.",
    "هذا الثبات في المستوى ليس سهلاً.",
    "تلاحظ الفرق عندما يفخر الناس بعملهم.",
    "أنا دقيق في هذه الأمور، وخرجت بلا ملاحظات.",
    "رفعت سقف توقعاتي ومع ذلك تجاوزوه.",
    "الأشياء البسيطة، منجزة بإتقان.",
    "خرجت بمزاج أفضل مما دخلت، وهذا يعني الكثير.",
    "لو كنت أسكن أقرب لزرتهم أكثر مما ينبغي.",
    "حتى التفاصيل الصغيرة كانت محسوبة.",
  ],
  tails: [
    ...
    "{kw} كان من الأبرز.",
    "لا تفوّت {kw}.",
    "{kw} يستحق الذكر أيضاً.",
    "سأعود من أجل {kw} وحده.",
    "إشارة خاصة إلى {kw}.",
    "{kw} فاق توقعاتي.",
    "في المرة القادمة سأبدأ بـ {kw}.",
    "{kw} حسم الأمر بالنسبة لي.",
    "إن احترت، خذ {kw}.",
    "أضف {kw} إلى قائمتك أيضاً.",
    "{kw} كان نقطة مضيئة أخرى.",
    "سعيد أننا لم نفوّت {kw}.",
    "لم يبالغ من مدح {kw}.",
    "{kw} استحق مكانه في هذا التقييم.",
    "اترك مساحة لـ {kw}.",
    "{kw} أنهى الزيارة بشكل جميل.",
    "ستسأل عن {kw} لاحقاً، صدقني.",
    "حتى {kw} نال العناية نفسها.",
    "ذكرنا {kw} في طريق العودة أيضاً.",
    "لا ندم على {kw} إطلاقاً.",
    "{kw} قد يكون النجم الهادئ هنا.",
  ],
  microOpeners: [
    "كان عليّ ترك تقييم.",
    "ملاحظة سريعة.",
    "تقييم متأخر، لكنه واجب.",
    "باختصار:",
    "أكتب هذا والتجربة ما زالت حاضرة.",
    "لمن يتردد:",
  ],
  noKeywordMid: [
...
const EN_VERTICAL: Partial<Record<Vertical, Partial<PoolSet>>> = {
  ...
  cafe: {
    openersLong: [
      "Grabbed a {coffee|drink|quick coffee} at {store} and ended up staying {longer than planned|a good while longer|well past my plan}, which says something.",
  ...
  aesthetic: {
    ...
    bridgesLong: [
      ...
      "Everything was explained before anything was done, which is exactly how it should be.",
    ],
  ...
  hotel: {
    openersLong: [
      "Stayed at {store} for a few nights and it got the important things right without any fuss.",
    ...
    coresLong: [
      "What made the stay was {list}. At {store} the small things added up.",
      "The thing I'd mention is {list}. After a long day of travelling, {store} had the basics exactly right.",
    ],
...
const JA_VERTICAL: Partial<Record<Vertical, Partial<PoolSet>>> = {
  restaurant: {
    openersLong: [
      "先日{store}で食事をしました。印象に残ったことを書いておきます。",
      "{store}で食事をして、{帰り道で思わず|家に着く前に|その日のうちに}感想を書きたくなりました。",
    ],
    openersShort: ["{store}、また食べに行きたいです。", "手短に{store}の感想を。良い食事で、印象的な一皿もありました。"],
    coresLong: [
      // 食べ物専用の述語（美味しい／食べる）を持つ行は、{list} が料理名のときだけ
      // 使われる（engine の filterTasteVoice が非飲食キーワードでは落とす）。
      // 3本目はゲートに依存しない安全形。
      "{list}が特に美味しかったです。",
      "{list}が最高で、また食べに来たいです。",
      "特に良かったのは{list}です。料理も{一つひとつ丁寧|どれも丁寧な仕上がり|全体的にレベルが高め}でした。",
      "{list}は看板に偽りなしでした。",
      "{list}を一口食べて、また来ることが決まりました。",
    ],
    bridgesLong: [
      "量の割に価格も{良心的で|納得感があり|きちんとしていて}、料理も{テンポよく|ちょうどいい間隔で|待たされることなく}出てきました。",
      "提供の速さと味のバランスが良かったです。",
      "メニュー選びに迷っていたら、さりげなく好みを聞いてくれました。",
    ],
    closersLong: [
      "お腹を空かせて、また{store}に伺いたいです。",
      "次は{別のメニュー|気になっていた他のメニュー|違う一品}を試しに{store}へ行きます。",
    ],
  },
  cafe: {
  ...
  aesthetic: {
    ...
    coresLong: [
      ...
      "挙げたいのは{list}で、{store}は速さより安全を優先しているのが伝わりました。",
    ],
  ...
  hotel: {
    openersLong: [
      "{store}に数泊しましたが、派手さはない分、大事なところがきちんとした滞在でした。",
      "よく調べずに{store}を予約しましたが、結果的に気楽で快適に過ごせました。",
    ],
    openersShort: ["{store}は快適で手間のかからない滞在で、また泊まりたいです。"],
    coresLong: [
      "滞在を良くしたのは{list}でした。{store}は目立つ何かより、細かい部分の丁寧さが印象に残りました。",
      "挙げたいのは{list}で、疲れて着いた日にはこういう配慮が本当にありがたいです。",
    ],
...
const AR_VERTICAL: Partial<Record<Vertical, Partial<PoolSet>>> = {
  restaurant: {
    ...
    coresLong: [
      // AR has no taste-voice gate (no attribute detector for Arabic), so a
      // food-only predicate ("delicious") must never attach directly to {list} —
      // it can hold seating / pricing / service keywords.
...
const EN_AGENCY: PoolSet = {
  openersLong: [
    "We've been working with {store} for a few months now and it's been one of the {easier|better|smarter} business decisions I've made.",
    "Hired {store} to sort out our {online presence|Google presence|visibility} and I'm glad we did.",
    "As a small business owner I was {wary|sceptical|tired} of marketing companies, but {store} changed my mind.",
    ...
    "We moved to {store} from a {bigger|more famous|pricier} agency and the service got {better, not worse|better, somehow|noticeably better}.",
    "Every meeting with {store} ends with me knowing exactly what happens next.",
    "Our shop was invisible online until we brought {store} in.",
  ...
  coresLong: [
    ...
    "On {list}, {store} really showed their expertise.",
    ...
    "They set up {list} properly, which is more than our old provider managed.",
  ],
  coresCompact: [
  ...
  bridgesLong: [
    "They answer {quickly|fast|the same day} on WhatsApp and nothing gets {lost or forgotten|dropped|missed}.",
    "Everything was explained in {plain language|words I actually understand|normal English}, no jargon and no pressure.",
    "Their pricing is {published on their website|flat and public|out in the open}, so there were no surprises on the invoice.",
    "The monthly report {actually arrives|lands on time|turns up without chasing}, and it shows {real numbers|actual results|the numbers that matter} rather than {fluff|filler|vanity metrics}.",
    "They tell you {honestly|upfront|straight} what they can and cannot promise, which I {respect|value|appreciate}.",
    "Communication has been {clear|straightforward|easy} from {day one|the first call|the start}.",
    "They treat a small business with the same {care|attention|respect} a big client would get.",
...
const JA_AGENCY: PoolSet = {
  ...
  fillers: [
    "ここまで率直な業者はなかなかありません。",
    "約束したことをきちんとやってくれる、当たり前のようで貴重です。",
...
const AR_AGENCY: PoolSet = {
  ...
  tails: [
    ...
    "{kw} من أفضل ما حصلنا عليه.",
    "راضون عن {kw} كذلك.",
    "{kw} فوق ما توقعنا.",
    "اسألوهم عن {kw}.",
    "من أبرز النقاط عندنا: {kw}.",
